...
import { Clock, Save, RefreshCw, Info, RotateCcw } from 'lucide-react'
...
interface UserWorkingHoursSettingsProps {
  userId?: string
}
...
export function UserWorkingHoursSettings({ userId }: UserWorkingHoursSettingsProps) {
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [workingHours, setWorkingHours] = useState<Record<string, WorkingHoursData>>({})
  const [defaultWorkingHours, setDefaultWorkingHours] = useState<Record<string, WorkingHoursData>>({})
  ...
  const fetchWorkingHours = async () => {
    try {
      ...
      // First, fetch default working hours from settings
      ...
      const defaultHoursMap: Record<string, WorkingHoursData> = {}
      defaultData?.forEach((item: any) => {
        defaultHoursMap[item.day] = {
          day: item.day,
          ...
          half_day_hours: item.half_day_hours,
          overtime_hours: item.overtime_hours
        }
      })
      setDefaultWorkingHours(defaultHoursMap)

      // If userId is provided, try to fetch user-specific working hours
      if (userId) {
        ...
        if (userData && userData.length > 0) {
          const userHoursMap: Record<string, WorkingHoursData> = {}
          userData.forEach((item: any) => {
            userHoursMap[item.day] = {
              id: item.id,
          ...
          setWorkingHours(userHoursMap)
          setHasCustomSettings(true)
        } else {
          // No user-specific settings, use defaults
          setWorkingHours(defaultHoursMap)
  ...
  const resetToDefaults = () => {
    if (confirm('Are you sure you want to reset to default working hours? This will discard all custom settings for this user.')) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Delete existing user-specific settings
      await supabase
        .from('user_working_hours_settings')
        .delete()
        .eq('user_id', userId)

      // Insert new user-specific settings
      const inserts = Object.values(workingHours).map(hours => ({
        user_id: userId,
        day: hours.day,
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-blue-800">
              <p className="font-medium mb-1">Using Default Settings</p>
              <p>This user is currently using the default working hours from Settings. You can customize them below and save to create user-specific settings.</p>
            </div>
          </div>
        </div>
      )}
