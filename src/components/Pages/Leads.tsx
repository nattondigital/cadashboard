...
export function Leads() {
  ...
  const fetchLeads = async () => {
    ...
    try {
      let query = supabase
        ...
        .select(`
          *,
          admin_users!leads_assigned_to_fkey(
            full_name
          )
        `)
      ...
      const leadsWithNames = data?.map((lead: any) => ({
        ...lead,
        assigned_to_name: lead.admin_users?.full_name || null
      })) || []

      console.log('📊 Fetched leads:', leadsWithNames?.length || 0)
  ...
  const filteredLeads = leads.filter(lead => {
    ...
    const matchesAssignedTo = !assignedToFilter || (lead.assigned_to_name || 'Unassigned') === assignedToFilter
        ...
        try {
          ...
          headers.forEach((header, index) => {
            ...
            if (value !== undefined && value !== '') {
              ...
              else if (header.includes('owner')) leadData.assigned_to = String(value)
  ...
  if (view === 'add') {
    return (
      <div className="md:static fixed inset-0 z-50 bg-gradient-to-br from-orange-50 to-red-50 md:bg-white flex flex-col md:block md:p-6">
        {/* Mobile Header */}
                  ...
                  <Card className="md:shadow-sm shadow-lg border-0 md:border rounded-2xl md:rounded-lg">
                    ...
                    <CardContent className="md:p-6 p-5">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
                        <div className="bg-gradient-to-br from-orange-50 to-red-50 md:bg-transparent md:p-0 p-4 md:rounded-none rounded-xl">
  ...
  return (
    <div className="md:p-6 md:p-4 p-0 bg-white min-h-screen md:min-h-0">
      <div className="hidden md:block mb-6">
        <PageHeader
      ...
      {loading ? (
        ...
        <>
          ...
          >
            ...
            <div className="hidden md:flex ml-auto items-center gap-2 border border-gray-300 rounded-md overflow-hidden">
              <button
                      ...
                      {filteredLeads.length === 0 ? (
                        ...
                        filteredLeads.map((lead, index) => (
                          ...
                          >
                            ...
                            <td className="py-3 px-4">
                              <span className="text-sm text-gray-600">{lead.assigned_to_name || 'Unassigned' || '-'}</span>
                            </td>
                            <td className="py-3 px-4">
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
